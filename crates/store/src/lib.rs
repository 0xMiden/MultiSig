...
impl MultisigStore {
	...
	pub async fn get_contract_info(&self, contract_id: &str) -> Result<Option<ContractInfo>> {
		...
		let MultisigContractRecord { contract_id, threshold, kind, created_at } = contract;
			...
			.transaction(|conn| {
				Box::pin(async move {
					...
					.await?
					{
						...
						let new_tx_sig =
							NewTxSigRecord { tx_id, approver_address, sig, created_at: None };
	...
	pub async fn get_transaction_signatures(
		...
		store::fetch_tx_sigs_by_tx_id(&mut self.get_conn().await?, tx_id)
			...
			.into_iter()
			.map(
				|TxSigRecord { tx_id, approver_address, sig, .. }| SignatureRecord {
					tx_id,
					approver_address,
					sig,
				},
