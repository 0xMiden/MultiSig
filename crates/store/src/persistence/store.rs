...
pub async fn save_new_contract_tx(
	...
	diesel::insert_into(schema::contract_tx::table).values(new_tx).execute(conn).await?;
...
pub async fn save_new_tx_sig(conn: &mut DbConn, new_tx_sig: NewTxSigRecord<'_>) -> Result<()> {
	diesel::insert_into(schema::tx_sig::table).values(new_tx_sig).execute(conn).await?;
